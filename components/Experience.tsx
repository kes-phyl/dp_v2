      ...
      <ol className='w-[50%]'>
        {responsibilities.map((responsibility, index) => (
          <li className='mb-4 flex' key={index}>
            <span className='text-2xl'><LiaCaretRightSolid className='text-[#610000] mr-10'/></span><p className='text-xs'>{responsibility.description}</p>         
          </li>
        ))}
      </ol>
    </div>
  ...
  return (
    <div>
      {companies.map((company, index) => (
        <button className='mr-12 block text-xs border-l tries text-[#708090] p-4 focus:border-[#00c1f5] focus:text-[#00c1f5] focus:bg-[#002244]' key={index} onClick={() => onSelectCompany(company)}>
          {company.name}
        </button>
      ))}
    </div>
  ...
  return (
   <section className='h-[100vh] content-center'>
    <div className='flex mb-10'>
         <div className='ml-[400px] text-3xl text-[#708090] flex mr-10'>
